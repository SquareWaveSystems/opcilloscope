- should hardcode 'opc.tcp://' in the connection dialog and handle it intelligently when pasted

- should auto populate address for connect after disconnect

- in the config file make this: 
192.168.1.6750000_202601261648.cfg
into this

192.168.1.67-50000_20260126_1648.cfg
- enter also needs to expand the node as well as subscribe. no key to expand the node?
...
-there is a bug with left clicks in the monitored variables pane. It is selecting/deselecting seemingly random variables. Instad it should only tick the Sel box when clicked there, otherwise only select the line. e.g. click to select the line, spacxe to scelect for recording.

-  M opens file menu, but F is suffcient it 
