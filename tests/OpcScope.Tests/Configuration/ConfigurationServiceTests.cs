...
namespace OpcScope.Tests.Configuration;
...
public class ConfigurationServiceTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"OpcScopeTests_{Guid.NewGuid()}");
    ...
    public async Task SaveAsync_ThenLoadAsync_RoundtripsConfig()
    {
        ...
        var filePath = Path.Combine(_tempDir, "test.opcscope");
    ...
    public async Task SaveAsync_ThenLoadAsync_PreservesMonitoredNodes()
    {
        ...
        var filePath = Path.Combine(_tempDir, "nodes.opcscope");
    ...
    public async Task SaveAsync_ThenLoadAsync_PreservesMetadata()
    {
        ...
        var filePath = Path.Combine(_tempDir, "meta.opcscope");
    ...
    public async Task SaveAsync_UpdatesLastModified()
    {
        ...
        var filePath = Path.Combine(_tempDir, "modified.opcscope");
    ...
    public async Task SaveAsync_SetsCurrentFilePath()
    {
        // Arrange
        var config = CreateTestConfig();
        var filePath = Path.Combine(_tempDir, "path.opcscope");
    ...
    public async Task SaveAsync_ClearsUnsavedChanges()
    {
        // Arrange
        var config = CreateTestConfig();
        var filePath = Path.Combine(_tempDir, "clean.opcscope");
        _service.MarkDirty();
    ...
    public async Task LoadAsync_SetsCurrentFilePath()
    {
        // Arrange
        var config = CreateTestConfig();
        var filePath = Path.Combine(_tempDir, "load.opcscope");
        await _service.SaveAsync(config, filePath);
        _service.Reset();
    ...
    public async Task LoadAsync_ClearsUnsavedChanges()
    {
        // Arrange
        var config = CreateTestConfig();
        var filePath = Path.Combine(_tempDir, "loadclean.opcscope");
        await _service.SaveAsync(config, filePath);
        _service.MarkDirty();
    ...
    {
        // Arrange
        var filePath = Path.Combine(_tempDir, "invalid.opcscope");
    ...
    {
        // Arrange
        var filePath = Path.Combine(_tempDir, "negative.opcscope");
        var json = """
        {
            "version": "1.0",
    ...
    {
        // Arrange
        var filePath = Path.Combine(_tempDir, "emptynode.opcscope");
        var json = """
        {
            "version": "1.0",
            "server": { "endpointUrl": "opc.tcp://localhost:4840" },
    ...
    {
        // Arrange
        var filePath = Path.Combine(_tempDir, "nonexistent.opcscope");
    ...
    public void Reset_ClearsCurrentFilePath()
    {
        ...
        _service.GetType().GetProperty("CurrentFilePath")!
            .SetValue(_service, "/some/path.opcscope");
    ...
    public async Task GetDisplayName_WithFile_ReturnsFileName()
    {
        // Arrange
        var config = CreateTestConfig();
        var filePath = Path.Combine(_tempDir, "myconfig.opcscope");
        await _service.SaveAsync(config, filePath);
    ...
    private static OpcScopeConfig CreateTestConfig()
    {
